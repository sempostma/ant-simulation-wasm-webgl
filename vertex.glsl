...
void main()
{
  if (v_renderMode == 1) {
    ...
    vec4 firstComp = texture2D(v_antsTexture, vec2((antIndex * 2.0) / float(ANTS_DATA_TOTAL) + 0.0125, 0.5));
    vec4 secondComp = texture2D(v_antsTexture, vec2(((antIndex * 2.0 + 1.0)) / float(ANTS_DATA_TOTAL - 1) + 0.0125, 0.5));
    ...
    float x = (xHO * 255.0 + xLO) * scaleDiv * 256.0 / WINDOW_WIDTH * 2.0;
    float y = (yHO * 255.0 + yLO) * scaleDiv * 256.0 / WINDOW_HEIGHT * 2.0;
